...
class TestMailingListMemberSelfRejectView(TapirIntegrationTest):
    @classmethod
    ...
    def test_post_mailingListsDisabled_returns403(self):
        ...
        response = self.client.post(
            reverse("core:member_self_reject"),
            data={"member_id": member.id, "list_name": "test_name@example.com"},
        )

        self.assertStatusCode(response, status.HTTP_403_FORBIDDEN)
        mailing_list.reject_request.assert_not_called()

    def test_post_loggedInUserIsTargetMember_rejects(self):
        member = MemberFactory.create(is_superuser=False)
        self.client.force_login(member)
        ...
        mailing_list.reject_request.assert_called_once_with(
            request_id=mailing_list.requests[0]["token"]
        )

    def test_post_loggedInUserIsAdmin_rejects(self):
