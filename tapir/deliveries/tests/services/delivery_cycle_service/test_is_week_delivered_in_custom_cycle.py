import datetime
from unittest.mock import Mock

from tapir.deliveries.services.delivery_cycle_service import DeliveryCycleService
from tapir.wirgarten.tests.test_utils import TapirUnitTest


class TestIsWeekDeliveredInCustomCycle(TapirUnitTest):
    @staticmethod
    def _build_cache(date, product_type, growing_period, delivered_weeks):
        return {
            "growing_periods_by_date": {date: growing_period},
            "delivery_weeks_by_product_type_and_growing_period": {
                product_type: {growing_period: delivered_weeks}
            },
        }

    def test_isWeekDeliveredInCustomCycle_weekIsInDeliveredWeeks_returnsTrue(self):
        date = datetime.date(year=2025, month=3, day=10)  # week 11
        product_type = Mock()
        growing_period = Mock()
        cache = self._build_cache(
            date=date,
            product_type=product_type,
            growing_period=growing_period,
            delivered_weeks={11, 15, 19},
        )

        self.assertTrue(
            DeliveryCycleService.is_week_delivered_in_custom_cycle(
                date=date, product_type=product_type, cache=cache
            )
        )

    def test_isWeekDeliveredInCustomCycle_weekIsNotInDeliveredWeeks_returnsFalse(self):
        date = datetime.date(year=2025, month=3, day=10)  # week 11
        product_type = Mock()
        growing_period = Mock()
        cache = self._build_cache(
            date=date,
            product_type=product_type,
            growing_period=growing_period,
            delivered_weeks={12, 15, 19},
        )

        self.assertFalse(
            DeliveryCycleService.is_week_delivered_in_custom_cycle(
                date=date, product_type=product_type, cache=cache
            )
        )

    def test_isWeekDeliveredInCustomCycle_noWeeksInPeriodAndNoFutureDeliveries_returnsFalse(
        self,
    ):
        date = datetime.date(year=2025, month=3, day=10)  # week 11
        product_type = Mock()
        growing_period = Mock()
        cache = self._build_cache(
            date=date,
            product_type=product_type,
            growing_period=growing_period,
            delivered_weeks=set(),
        )

        self.assertFalse(
            DeliveryCycleService.is_week_delivered_in_custom_cycle(
                date=date, product_type=product_type, cache=cache
            )
        )

    def test_isWeekDeliveredInCustomCycle_dateIsNotAMonday_returnsTrue(self):
        date = datetime.date(year=2025, month=3, day=12)  # Wednesday, still week 11
        product_type = Mock()
        growing_period = Mock()
        cache = self._build_cache(
            date=date,
            product_type=product_type,
            growing_period=growing_period,
            delivered_weeks={11, 15, 19},
        )

        self.assertTrue(
            DeliveryCycleService.is_week_delivered_in_custom_cycle(
                date=date, product_type=product_type, cache=cache
            )
        )
