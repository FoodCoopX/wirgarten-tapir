...
class TestGetDatetimeOfLatestMonthlyExport(SimpleTestCase):
    def setUp(self):
        mock_timezone(
            self, datetime.datetime(year=2024, month=1, day=5, hour=8, minute=30)
        )

    def test_getDatetimeOfLatestMonthlyExport_dateThisMonthIsBeforeNow_returnsDateThisMonth(
        self,
    ):
        export = Mock()
        ...
        result = AutomatedExportsManager.get_datetime_of_latest_monthly_export(export)
    ...
    def test_getDatetimeOfLatestMonthlyExport_dateThisMonthIsAfterNow_returnsDatePreviousMonth(
        self,
    ):
        export = Mock()
