...
class TestGrowingPeriodChoiceProvider(TapirIntegrationTest):
    ...
    def test_getAvailableGrowingPeriods_onlyExistingPeriodIsInTheFuture_futurePeriodReturnedIgnoringThreshold(
        self,
    ):
        now = mock_timezone(self, now=datetime.datetime(year=2000, month=1, day=1))
        future_growing_period = GrowingPeriodFactory.create(
