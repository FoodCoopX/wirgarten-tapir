const getChecked = () => {
  ...
  const checkedCheckboxes = Array.from(checkboxes).filter((checkbox) =>
...
const handleCheckButton = () => {
  ...
  ConfirmationModal.open(
    "Bist du dir sicher?",
    `Möchtest du die <strong>${count}</strong> ausgewählten Zeichnungen auf <strong>geprüft</strong> setzen?`,
    "Ja, auf geprüft setzen",
    "danger",
    () => {
      window.location.href =
        "/tapir/admin/newcontracts/confirm" +
        Tapir.stringifyUrlParams(checkedCheckboxes);
    },
  );
};
