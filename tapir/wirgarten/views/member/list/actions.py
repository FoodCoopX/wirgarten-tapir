...
def export_coop_member_list(request, **kwargs):
    ...
    for entry in Member.objects.order_by("member_no"):
        ...
        data[KEY_COOP_SHARES_TRANSFER_FROM_TO] = "\n".join(
            map(
                lambda x: f"Übertragung {format_currency(abs(x.quantity) * get_parameter_value(key=ParameterKeys.COOP_SHARE_PRICE, cache=cache))} € {get_transaction_verb(x)} {x.transfer_member.first_name} {x.transfer_member.last_name} (Nr. {x.transfer_member.member_no})",
                transfers,
            )
        )
