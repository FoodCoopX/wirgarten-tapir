...
export function instanceOfGrowingPeriodWithDeliveryDayAdjustments(
  ...
  if (!("adjustments" in value) || value["adjustments"] === undefined)
    return false;
  if (
    !("maxJokersPerMember" in value) ||
    value["maxJokersPerMember"] === undefined
  )
    return false;
  if (
    ...
    return false;
  if (!("jokersEnabled" in value) || value["jokersEnabled"] === undefined)
    return false;
  if (
    !("isAvailableInBestellWizard" in value) ||
    value["isAvailableInBestellWizard"] === undefined
  )
    return false;
  return true;
}
