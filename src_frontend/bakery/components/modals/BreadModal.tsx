...
export const BreadModal: React.FC<BreadModalProps> = ({
  bread,
  csrfToken,
  onSave,
  onClose,
  ...
  useEffect(() => {
    loadLabels();
    if (bread) {
      const labelIds = (bread.labels || []).map((label) =>
        typeof label === "object" ? (label as any).id : label,
      );
      ...
      setFormData({
        ...
        oneBatchCanBeBakedInMoreThanOneStove:
          bread.oneBatchCanBeBakedInMoreThanOneStove || false,
  ...
  const addPiecesPerLayer = () => {
    ...
    if (
      !isNaN(value) &&
      value > 0 &&
      !formData.piecesPerStoveLayer?.includes(value)
    ) {
      setFormData((prev) => ({
        ...prev,
        piecesPerStoveLayer: [...(prev.piecesPerStoveLayer || []), value].sort(
          (a, b) => a - b,
        ),
      }));
      setPiecesInput("");
  ...
  const removePiecesPerLayer = (value: number) => {
    setFormData((prev) => ({
      ...prev,
      piecesPerStoveLayer: (prev.piecesPerStoveLayer || []).filter(
        (v: number) => v !== value,
      ),
    }));
  };
  ...
  const unassignedLabels = availableLabels.filter(
    (label) => !formData.labels!.includes(label.id!),
  );
          ...
          <div className="mb-3">
            <label className="form-label fw-bold">
              ...
              {formData.labels!.length > 0 && (
                <span className="badge badge-bakery-success ms-2">
            ...
            {loading ? (
              <div className="text-center py-3">
                <div
                  className="spinner-border spinner-border-sm spinner-bakery"
                  role="status"
                >
          ...
          <div className="border rounded p-3 bg-bakery-gray-light">
            ...
            <div className="mt-3">
              ...
              <div className="mb-3">
                ...
                >
                  Mögliche Anzahl Brote, die pro Etage gebacken werden können.{" "}
                  <br />
                  Um mehrere Zahlen einzugeben bzw. um zu speichern, bitte auf
                ...
                <div className="d-flex flex-wrap gap-2">
                  {(formData.piecesPerStoveLayer || []).map((value: number) => (
                    <span
                      key={value}
              ...
              <div className="form-check form-switch mb-3">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      oneBatchCanBeBakedInMoreThanOneStove: e.target.checked,
                    })
                  }
                />
                <label
                  className="form-check-label small"
                  htmlFor="oneBatchCanBeBakedInMoreThanOneStove"
                >
                  ...
                  <div className="mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          minRemainingPieces: e.target.value
                            ? parseInt(e.target.value)
                            : undefined,
                        })
                      }
                      ...
                      placeholder="z.B. 5"
                    />
                    <small className="text-muted">Für Laufkundschaft</small>
                  </div>
                </div>
              </div>
            </div>
