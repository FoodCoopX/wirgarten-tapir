...
export const AllocationTable: React.FC<AllocationTableProps> = ({
  ...
  function getRowSum(locationId: string): number {
    return sumValues(
      activeBreads.map((bread) => allocations[locationId]?.[bread.id!] ?? null),
    );
  }

  function getColSum(breadId: string): number {
    return sumValues(
      pickupLocations.map(
  ...
  function getTotalSum(): number {
    return pickupLocations.reduce(
      (total, location) => total + getRowSum(location.id),
      0,
    );
  }
                ...
                <tr key={location.id}>
                  ...
                  {activeBreads.map((bread) => (
                    <td key={bread.id} className="p-1">
                      {/* A browser reports unparseable number input
                          ("12-", "1e-") as an EMPTY value with
                          validity.badInput set. */}
                      <input
                        type="number"
                        min={0}
                        step={1}
