...
const FIELDS = [
  {
    key: "minPieces" as const,
    label: "Min. Stück",
    help: "Mindestanzahl die gesamt gebacken werden soll",
  },
  {
    key: "maxPieces" as const,
    label: "Max. Stück",
    help: "Maximale Anzahl die gesamt gebacken werden kann",
  },
  {
    key: "minRemainingPieces" as const,
    label: "Min. Rest",
    help: "Direkte Bestellungen von Cafés etc.",
  },
  {
    key: "fixedPieces" as const,
    label: "Fixe Stück",
    help: "exakte Anzahl für diesen Tag (wird im Backplan direkt so übernommen)",
  },
];
...
export const DailySettingsModal: React.FC<DailySettingsModalProps> = ({
      ...
      .then((existing) => {
        ...
        activeBreads.forEach((bread) => {
          const entry = existing.find(
            (e: BreadSpecificsPerDeliveryDay) => e.bread === bread.id,
          );
          initial[bread.id!] = {
            minPieces: entry?.minPieces != null ? String(entry.minPieces) : "",
            maxPieces: entry?.maxPieces != null ? String(entry.maxPieces) : "",
            minRemainingPieces:
              entry?.minRemainingPieces != null
                ? String(entry.minRemainingPieces)
                : "",
            fixedPieces:
              entry?.fixedPieces != null ? String(entry.fixedPieces) : "",
          };
        });
  ...
  const getPlaceholder = (
    bread: BreadList,
    field: keyof SpecificsData[string],
  ): string => {
    const fieldMap = {
  ...
  const handleSaveAndClose = () => {
    ...
    Object.entries(specifics).forEach(([breadId, values]) => {
      ...
      if (changed) {
        updates.push({
          ...
          minRemainingPieces:
            values.minRemainingPieces === ""
              ? null
              : Number(values.minRemainingPieces),
          fixedPieces:
            values.fixedPieces === "" ? null : Number(values.fixedPieces),
        });
      }
    });
    ...
    bakeryApi
      .bakeryBreadSpecificsBulkUpdateCreate({
      ...
      .catch((error) => {
        handleRequestError(
          error,
          "Fehler beim Speichern der Tageseinstellungen",
        );
      })
      .finally(() => {
        setSaving(false);
