...
export const LabelsModal: React.FC<BreadLabelsModalProps> = ({
  bread,
  csrfToken,
  onClose,
  onUpdate,
  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      .then(([breadData, labelsData]) => {
        const labelIds = (breadData.labels ?? []).map((label) => label.id);

        const activeLabels = labelsData.filter(
      ...
      <Modal.Body>
        {loading ? (
          <div className="text-center py-4">
            <div
              className="spinner-border text-bakery-primary-dark"
              role="status"
            >
