...
export const DistributionListSection: React.FC<
  DistributionListSectionProps
> = ({
  isOpen,
  onToggle,
  isLoading,
        ...
        <>
          {isLoading ? (
            ...
            <>
              {!hasSolverResults && (
                <div
                  className="alert alert-info alert-sm mb-2"
              ...
              >
                      ...
                      <>
                        {distBreadNames.map((name) => (
                          <th
                            key={name}
                            colSpan={3}
                            className="text-center border-left-bakery"
                            style={{
                              minWidth: "60px",
                              padding: "4px 2px",
                              fontSize: "0.6rem",
                ...
                <tbody>
                  {Object.entries(locationBreads)
                    .sort(([a], [b]) => a.localeCompare(b))
                    .map(([loc, breads]) => {
                      const locTots = hasSolverResults
                        ? {
                            totalBaked: Object.values(breads).reduce(
                              (s, b) => s + (b.baked || 0),
                              0,
                            ),
                            totalOrdered: Object.values(breads).reduce(
                              (s, b) => s + (b.ordered || 0),
                              0,
                            ),
                            totalExtra:
                              Object.values(breads).reduce(
                                (s, b) => s + (b.baked || 0),
                                0,
                              ) -
                              Object.values(breads).reduce(
                      ...
                      return (
                        <tr key={loc}>
                            ...
                            ? distBreadNames.map((name) => {
                                ...
                                return (
                                  <React.Fragment key={`${loc}-${name}`}>
                                    <td className="text-center border-left-bakery">
                                      {data.ordered || "—"}
                                    </td>
                                    <td
                                      className={`text-center ${data.extra > 0 ? "text-bakery-success" : data.extra < 0 ? "text-bakery-danger" : "text-bakery-muted"}`}
                                      style={{
                                        fontSize: "0.65rem",
                                      }}
                                    >
                                    ...
                                    <td className="text-center text-bakery-primary-darker fw-bold">
                                      {data.baked || "—"}
                                    </td>
                                  </React.Fragment>
                                );
                          ...
                          <td className="text-center border-left-bakery">
                            {locTots.totalOrdered}
                          </td>
                          <td
                            className={`text-center ${locTots.totalExtra > 0 ? "text-bakery-danger" : "text-bakery-muted"}`}
                            style={{
                              fontSize: "0.65rem",
                            }}
                          >
                            {hasSolverResults
                              ? locTots.totalExtra > 0
                                ? `+${locTots.totalExtra}`
                                : locTots.totalExtra === 0
                                  ? "—"
                                  : locTots.totalExtra
                  ...
                  <tr className="total-row-brown fw-bold">
                    ...
                    {hasSolverResults
                      ? distBreadNames.map((name) => {
                          const totBaked = Object.values(locationBreads).reduce(
                            (s, b) => s + (b[name]?.baked || 0),
                            0,
                          );
                          const totOrdered = Object.values(
                            locationBreads,
                          ).reduce((s, b) => s + (b[name]?.ordered || 0), 0);
                          ...
                          return (
                            <React.Fragment key={`total-${name}`}>
                              ...
                              <td
                                className={`text-center ${totExtra > 0 ? "text-bakery-success" : totExtra < 0 ? "text-bakery-danger" : "text-bakery-muted"}`}
                              >
                                {totExtra > 0
                                  ? `+${totExtra}`
                                  : totExtra === 0
                    ...
                    {(() => {
                      const gtBaked =
                        ...
                        Object.keys(locationTotals).length > 0
                          ? Object.values(locationTotals).reduce(
                              (s, t) => s + t.totalBaked,
                              0,
                            )
                          : Object.values(locationBreads).reduce(
                              (s, breads) =>
                                s +
                                Object.values(breads).reduce(
                                  (ss, b) => ss + (b.baked || 0),
                      ...
                      return (
                        <>
                          ...
                          <td
                            className={`text-center ${gtExtra > 0 ? "text-bakery-danger" : "text-bakery-muted"}`}
                          >
                            {hasSolverResults
                              ? gtExtra > 0
                                ? `+${gtExtra}`
                ...
                <p className="mb-0" style={{ fontSize: "0.7rem" }}>
                  {hasSolverResults ? (
                    <>
                      ...
                      = Bestellt &nbsp;|&nbsp;
                      <span className="text-bakery-success">+</span>/
                      <span className="text-bakery-danger">-</span> = Differenz
                      &nbsp;|&nbsp;
                      <span
                        className="material-icons"
                        style={{ fontSize: "10px", verticalAlign: "middle" }}
                      ...
                      = Gebacken (lt. Backplan)
                    </>
                  ) : (
                    <>
                      Best. = direkt gewählte Brotsorten &nbsp;|&nbsp; offen =
                      noch nicht gewählt &nbsp;|&nbsp; Ges. = Brotanteile gesamt
                    </>
                  )}
                </p>
              </div>
